...
class P:
    ...
    def _moves(self):
        ...
        while d:
            red, blue = d.popleft()
            rrow, rcol, rcount, rstring = red
            brow, bcol, bcount, bstring = blue

            for go_type, go in enumerate(G.ALL):
                nrcheck, nrrow, nrcol = self._move(rrow, rcol, go)
                nbcheck, nbrow, nbcol = self._move(brow, bcol, go)

                nrstring = rstring + G.GO_TYPE[go_type]
                nbstring = bstring + G.GO_TYPE[go_type]
                ...
                d.append(((nrrow, nrcol, rcount + 1, nrstring), (nbrow, nbcol, bcount + 1, nbstring)))
